...
```{r loading-data}
# Your code below:
pollution <- read_tsv(here("data", "madrid_pollution.tsv"))
```
...
#pollution_diff <- pollution %>% 
#  filter(year == 2001 & 2006) %>%
#  select(-mnth, -date) %>% 
#  pivot_longer(cols = "BEN" : "TOL", names_to = "pollutant") %>% 
#  map_df(~(.x, na.rm = TRUE))
